...
for month in active_months:
    download_xml("S2*MSIL2A*"+year+month+"*"+place+"* AND ( (platformname:Sentinel-2 AND cloudcoverpercentage:[0 TO 50.0])) ","month_"+month+".xml")
...
for j in range(len(product_list)):
    ...
    if(os.path.isdir("products/"+product_list[j])==False):
        ...
        if(os.path.isdir("data/"+product_list[j]+".SAFE")==True):
            ...
            nr_of_tiles=len([name for name in os.listdir(where) if os.path.isfile(os.path.join(where, name))])
            if(nr_of_tiles>0):
